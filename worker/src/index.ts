...
export function eventsToICal(
  events: any[],
  query: { lat: number; lng: number; category?: string }
): string {
  ...
  for (const event of events) {
    ...
    if (event.description) {
      lines.push(
        foldICalLine(`DESCRIPTION:${escapeICalText(event.description)}`)
      );
    }
    if (location) {
...
export default {
  async scheduled(_event: ScheduledEvent, env: Env): Promise<void> {
    ...
    if (env.BACKUP_BUCKET) {
      ...
      let objects = [...listed.objects].sort((a, b) =>
        a.key < b.key ? -1 : 1
      );
...
async function handleGetStats(env: Env): Promise<Response> {
  ...
  const lastResult = await env.DB.prepare(
    'SELECT title, venue_name, lat, lng, url FROM events ORDER BY created_at DESC LIMIT 1'
  ).first<{
    title: string;
...
async function handleGetEvents(request: Request, env: Env): Promise<Response> {
  ...
  if (!hasGeo && !pubkeyFilter) {
    ...
    const allResult = await env.DB.prepare(allQuery)
      .bind(...allParams)
      .all();
    const allEvents =
      allResult.results?.map(row => ({
        ...(row as DbEventRow),
        tags: (row as DbEventRow).tags
          ? JSON.parse((row as DbEventRow).tags as string)
          : [],
      })) || [];
  ...
  if (format === 'ical') {
    const icalContent = eventsToICal(filteredEvents, {
      lat,
      lng,
...
async function isBlocklisted(env: Env, pubkey: string): Promise<boolean> {
  const row = await env.DB.prepare(
    'SELECT pubkey FROM blocklist WHERE pubkey = ?'
  )
    .bind(pubkey)
    .first();
  return row !== null;
...
async function handlePostEvent(request: Request, env: Env): Promise<Response> {
  ...
  const { from: dedupFrom, to: dedupTo } = dedupSqlWindow(event.start_time);
  ...
  const existingEvents = await env.DB.prepare(
    ...
    .bind(...candidateCells, dedupFrom, dedupTo)
    .all();
  ...
  if (existingEvents.results && existingEvents.results.length > 0) {
    ...
    for (const existing of existingEvents.results) {
      ...
      if (dup) {
        console.log(
          `Duplicate event detected: "${event.title}" similar to existing "${row.title}" (ID: ${row.id})`
        );
        return jsonResponse(
          {
...
async function handleDeleteEvent(
  ...
  const { pubkey, signature } = (await request.json()) as {
    pubkey: string;
    signature: string;
  };
...
async function handleGetBlocklist(
  ...
  const result = await env.DB.prepare(
    'SELECT pubkey, created_at FROM blocklist ORDER BY created_at DESC'
  ).all();
  return jsonResponse(result.results || []);
}

async function handlePostBlocklist(
  request: Request,
  env: Env
): Promise<Response> {
  if (!env.ADMIN_PUBKEY) {
  ...
  if (!pubkey || !signature || !target_pubkey) {
    return jsonResponse(
      { error: 'Missing pubkey, signature, or target_pubkey' },
      400
    );
  }
  ...
  const isValid = await verifyAdminSignature(
    target_pubkey,
    env.ADMIN_PUBKEY,
    signature
  );
  ...
  await env.DB.prepare(
    'INSERT OR IGNORE INTO blocklist (pubkey, created_at) VALUES (?, ?)'
  )
...
async function handleDeleteBlocklist(
  ...
  await env.DB.prepare('DELETE FROM blocklist WHERE pubkey = ?')
    .bind(targetPubkey)
    .run();
