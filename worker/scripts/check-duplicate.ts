...
if (localIdx === -1 && remoteIdx === -1) {
  console.error(
    'Usage: npx tsx scripts/check-duplicate.ts --local|--remote <id1> <id2>'
  );
  process.exit(1);
}
...
function fetchEvents(id1: string, id2: string, remote: boolean): DbEvent[] {
  ...
  const cmd = `npx wrangler d1 execute tokoro-db ${flag} --json --command "${sql}"`;
  ...
  if (!Array.isArray(results)) {
    console.error(
      'Unexpected wrangler output shape:',
...
async function main() {
  ...
  if (!a) {
    console.error(`Event not found: ${id1}`);
    process.exit(1);
  }
  if (!b) {
    console.error(`Event not found: ${id2}`);
    process.exit(1);
  }
  ...
  console.log(
    `    B in A's neighborhood: ${inNeighborhood ? 'YES' : 'NO ← would be missed by old single-cell query'}`
  );
  if (!inNeighborhood) {
    console.log(
  ...
  console.log(`\n[2] Distance`);
  console.log(
    `    Haversine: ${(distKm * 1000).toFixed(1)} m  (threshold: ${DEDUP_DISTANCE_KM * 1000} m)`
  );
  ...
  console.log(`\n[3] Time difference`);
  console.log(
    `    |start_time A − start_time B|: ${timeDiffMin.toFixed(1)} min  (threshold: ${DEDUP_TIME_WINDOW_MS / 60000} min)`
  );
  ...
  console.log(
    `    Similarity: ${titleSim.toFixed(3)}  (fast-path threshold: ≥ ${LEVENSHTEIN_FAST_PATH})`
  );
  ...
  if (apiKey) {
    ...
    console.log(
      `    Levenshtein similarity: ${titleSim.toFixed(3)}  (fallback threshold: ≥ ${LEVENSHTEIN_FALLBACK})`
    );
