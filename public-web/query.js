...
(function initRelayMode() {
  if (new URLSearchParams(window.location.search).get('relay') !== '1') return;

  Array.from(document.body.children).forEach(function (el) {
    if (el.id !== 'relay-ui') el.style.display = 'none';
  });
  const relayUi = document.getElementById('relay-ui');
  relayUi.style.display = 'flex';

  const stEl = document.getElementById('relay-st');
  const eventsEl = document.getElementById('relay-events');
  const actsEl = document.getElementById('relay-acts');
  const pubBtn = document.getElementById('relay-pub');
  const cancelBtn = document.getElementById('relay-cancel');
  ...
  const cfgFields = document.getElementById('relay-cfg-fields');
  const cfgToggle = document.getElementById('relay-cfg-toggle');
  const rlyCfgApiKey = document.getElementById('rly-apikey');
  const rlyCfgWorker = document.getElementById('rly-worker');
  const rlyCfgApiUrl = document.getElementById('rly-apiurl');
  const rlyCfgPrivKey = document.getElementById('rly-privkey');
  const rlyCfgPubKey = document.getElementById('rly-pubkey');

  rlyCfgApiKey.value = localStorage.getItem('tokoro_api_key') || '';
  rlyCfgWorker.value =
    localStorage.getItem('tokoro_worker_url') ||
    (typeof DEFAULT_CRAWLER_URL !== 'undefined' &&
    !DEFAULT_CRAWLER_URL.startsWith('__')
      ? DEFAULT_CRAWLER_URL
      : '');
  rlyCfgApiUrl.value =
    localStorage.getItem('tokoro_api_url') ||
    (typeof API_URL !== 'undefined' && !API_URL.startsWith('__')
      ? API_URL
      : '');

  const cfgSaveBtn = document.getElementById('relay-cfg-save');

  function updateCfgVisibility() {
    const allSet =
      rlyCfgApiKey.value.trim() &&
      rlyCfgWorker.value.trim() &&
      rlyCfgApiUrl.value.trim();
    cfgFields.style.display = allSet ? 'none' : 'flex';
    cfgSaveBtn.style.display = allSet ? 'none' : 'block';
    cfgToggle.style.display = allSet ? 'block' : 'none';
  }
  updateCfgVisibility();
  ...
  cfgToggle.addEventListener('click', function (e) {
    ...
    cfgSaveBtn.style.display = 'block';
    cfgToggle.style.display = 'none';
  });

  function saveRelaySettings() {
    localStorage.setItem('tokoro_api_key', rlyCfgApiKey.value);
    localStorage.setItem('tokoro_worker_url', rlyCfgWorker.value);
    localStorage.setItem('tokoro_api_url', rlyCfgApiUrl.value);
    updateCfgVisibility();
    maybeRunBufferedCrawl();
  }

  document
    .getElementById('relay-cfg-save')
    .addEventListener('click', saveRelaySettings);

  function handleSettingsEnter(e) {
    if (e.key === 'Enter') saveRelaySettings();
  }

  rlyCfgApiKey.addEventListener('input', function () {
    localStorage.setItem('tokoro_api_key', rlyCfgApiKey.value);
    maybeRunBufferedCrawl();
  });
  rlyCfgApiKey.addEventListener('keydown', handleSettingsEnter);
  rlyCfgWorker.addEventListener('input', function () {
    localStorage.setItem('tokoro_worker_url', rlyCfgWorker.value);
    maybeRunBufferedCrawl();
  });
  rlyCfgWorker.addEventListener('keydown', handleSettingsEnter);
  rlyCfgApiUrl.addEventListener('input', function () {
    localStorage.setItem('tokoro_api_url', rlyCfgApiUrl.value);
    maybeRunBufferedCrawl();
  });
  rlyCfgApiUrl.addEventListener('keydown', handleSettingsEnter);

  rlyCfgPrivKey.addEventListener('change', async function () {
    const privB64 = rlyCfgPrivKey.value.trim();
    if (!privB64) return;
    try {
      ...
      const pubBytes = Uint8Array.from(atob(pubB64std), c => c.charCodeAt(0));
      const pubHex = Array.from(pubBytes)
        .map(b => b.toString(16).padStart(2, '0'))
        .join('');
      rlyCfgPubKey.textContent = pubHex;
      localStorage.setItem(
        'tokoro_keypair',
        JSON.stringify({ pubkey: pubHex, privkeyB64: privB64 })
      );
    } catch (e) {
      setSt(LANG.invalidPrivKey(e.message), 'err');
    }
  });

  function populateKeypairFields(kp) {
    rlyCfgPrivKey.value = kp.privkeyB64;
    rlyCfgPubKey.textContent = kp.pubkey;
  }

  function maybeRunBufferedCrawl() {
    if (!crawlData || crawlRunning) return;
    const apiKey = (localStorage.getItem('tokoro_api_key') || '').trim();
    const workerUrl = (localStorage.getItem('tokoro_worker_url') || '').trim();
    if (apiKey && workerUrl) {
      updateCfgVisibility();
      runCrawl(crawlData);
    }
  ...
  const noticeDismiss = document.getElementById('relay-notice-dismiss');
  if (noticeDismiss) {
    noticeDismiss.addEventListener('click', function (e) {
      e.preventDefault();
      document.getElementById('relay-notice').classList.remove('show');
    });
  }

  async function getRelayKeypair() {
    const kp = await loadOrCreateKeypair();
    populateKeypairFields(kp);
    return kp;
  }

  function getSelectedRelayEvents() {
    return Array.from(eventsEl.querySelectorAll('.relay-card__cb:checked')).map(
      function (cb) {
        return extractedEvents[parseInt(cb.dataset.idx, 10)];
      }
    );
  }

  function updatePubBtn() {
    var total = extractedEvents.length;
    var selected = eventsEl.querySelectorAll('.relay-card__cb:checked').length;
    if (selected === 0) {
      pubBtn.disabled = true;
      pubBtn.textContent = LANG.publishBtn;
    } else if (selected < total) {
      pubBtn.disabled = false;
      pubBtn.textContent = LANG.publishNBtn(selected);
    } else {
      pubBtn.disabled = false;
      pubBtn.textContent = LANG.publishBtn;
    }
  ...
  function setSt(msg, cls) {
    ...
    stEl.className = 'relay-st ' + cls;
  }

  if (!window.opener) {
    setSt(LANG.noOpener, 'err');
    return;
  }
  ...
  getRelayKeypair()
    .then(function (kp) {
      if (kp.isNew) {
        const noticeEl = document.getElementById('relay-notice');
        if (noticeEl) noticeEl.classList.add('show');
      }
      window.opener.postMessage({ type: 'ready' }, '*');
    })
    .catch(function (err) {
      setSt(LANG.keypairError(err.message || err), 'err');
    });
  ...
  window.addEventListener('message', async function (evt) {
    ...
    crawlData = evt.data;
    await runCrawl(crawlData);
  });

  async function runCrawl(data) {
    ...
    if (!workerUrl || !apiKey) {
      cfgFields.style.display = 'flex';
      cfgToggle.style.display = 'none';
      setSt(LANG.missingSettings, 'err');
      return;
    }
    crawlRunning = true;
    setSt(LANG.extractingEvents, 'info');
    try {
      ...
      if (result.cleaned_text) {
        console.log(
          '[tokoro] cleaned text sent to LLM (' +
            result.cleaned_text.length +
            ' chars):',
          result.cleaned_text
        );
      }
      if (!resp.ok || !result.success) {
        setSt(result.message || result.error || 'Unknown error', 'err');
        return;
      }
      ...
      extractedEvents = evs;
      setSt(LANG.foundEvents(evs.length), 'ok');
      eventsEl.innerHTML = evs
        .map(function (e, i) {
          return (
            '<div class="relay-card">' +
            '<input type="checkbox" class="relay-card__cb" data-idx="' +
            i +
            '" checked aria-label="Select ' +
            escHtml(e.title || LANG.untitled) +
            '">' +
            '<h4>' +
            escHtml(e.title || LANG.untitled) +
            '</h4>' +
            ...
            '</p>' +
            (e.venue_name ? '<p>' + escHtml(e.venue_name) + '</p>' : '') +
            (e.address
              ? '<p class="relay-addr">' + escHtml(e.address) + '</p>'
              : '') +
            '<p class="relay-cat">' +
        ...
        .join('');
      eventsEl.querySelectorAll('.relay-card__cb').forEach(function (cb) {
        cb.addEventListener('change', updatePubBtn);
      });
      updatePubBtn();
      actsEl.style.display = 'flex';
    } catch (e) {
      setSt(LANG.networkError(e.message), 'err');
    } finally {
      crawlRunning = false;
    }
  }
  ...
  pubBtn.addEventListener('click', async function () {
    var selectedEvents = getSelectedRelayEvents();
    if (!selectedEvents.length) {
      setSt(LANG.noEventsSelected, 'err');
      return;
    }

    const storedApiUrl = (localStorage.getItem('tokoro_api_url') || '').trim();
    var apiUrl = storedApiUrl
      ? storedApiUrl.replace(/\/$/, '')
      : typeof API_URL !== 'undefined' && API_URL && !API_URL.startsWith('__')
        ? API_URL.replace(/\/$/, '')
        : null;
    if (!apiUrl) {
      setSt(LANG.apiUrlMissing, 'err');
      cfgFields.style.display = 'flex';
      cfgToggle.style.display = 'none';
      return;
    }
    ...
    pubBtn.textContent = LANG.publishingBtn;
    setSt(LANG.publishingStatus, 'info');

    try {
      const kp = await getRelayKeypair();
      let published = 0;
      let failed = 0;
      ...
      for (const event of selectedEvents) {
        try {
          ...
          console.error('Network error publishing "' + event.title + '":', e);
          failed++;
        }
      }

      if (failed === 0) {
        setSt(LANG.publishedOk(published), 'ok');
        eventsEl.innerHTML = '';
        actsEl.style.display = 'none';
      } else {
        setSt(LANG.publishedPartial(published, failed), 'err');
      }
    } catch (e) {
      setSt(LANG.publishError(e.message), 'err');
    } finally {
      pubBtn.disabled = cancelBtn.disabled = false;
      pubBtn.textContent = LANG.publishSelectedBtn;
    }
  });
